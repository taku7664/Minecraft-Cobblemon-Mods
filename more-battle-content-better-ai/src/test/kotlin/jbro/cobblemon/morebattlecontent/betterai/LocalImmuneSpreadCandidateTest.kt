...
class LocalImmuneSpreadCandidateTest {
    ...
    @Test
    fun `spread actor can be in slot one with canonical component order`() {
        checkCoverage(setOf("flying"), true, spreadSlot = 1)
        checkCoverage(emptySet(), false, spreadSlot = 1)
    }
    ...
    private fun checkCoverage(partnerTypes: Set<String>, expected: Boolean, partnerAbility: String? = null, opponentCount: Int = 2,
        spreadSlot: Int = 0) {
        val partnerSlot = 1 - spreadSlot
        val allies = listOf(mon(BattleSide.ALLY, spreadSlot, setOf("normal")), mon(BattleSide.ALLY, partnerSlot, partnerTypes, partnerAbility))
        val foes = (0 until opponentCount).map { mon(BattleSide.OPPONENT, it, setOf("normal")) }
        val spread = BattleActionCandidate("spread", BattleActionKind.USE_MOVE, actorSlot = spreadSlot, moveSlot = 0,
        ...
        val joints = (listOf(spread) + attacks(spreadSlot)).flatMap { a -> attacks(partnerSlot).map { b ->
            val parts = listOf(a, b).sortedBy { it.actorSlot }
            BattleActionCandidate(parts.joinToString("+") { it.actionId }, BattleActionKind.COMPOSITE,
        ...
        val cooperative = ranked.first { it.outcome.candidate.componentActions.any { part -> part.actionId == "spread" } }.outcome.candidate
        val targets = cooperative.componentActions.single { it.actionId == "spread" }.facts!!.spreadTargets
