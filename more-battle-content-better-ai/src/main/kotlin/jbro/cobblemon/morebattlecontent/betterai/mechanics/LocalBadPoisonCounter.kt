 ...
 *
 * Takes the previous per-Pokemon counts rather than the whole recursive history: it only ever read one
 * map off it, and depending on the history type made a pure mechanics helper part of the search's
 * dependency cycle for no benefit.
 */
internal object LocalBadPoisonCounter {
    ...
    fun advance(
        ...
        previousTurnsByPokemon: Map<UUID, Int>,
        ...
        .associate { pokemon ->
            ...
            val completed = if (stayedIn) {
                (previousTurnsByPokemon[pokemon.battlePokemonId] ?: 0) + 1
            } else {
                1
            }
