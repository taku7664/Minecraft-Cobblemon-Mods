...
import jbro.cobblemon.morebattlecontent.betterai.mechanics.PublicSwitchIncomingThreatCalculator
    ...
    private fun digest(open: BattleBrainOpenContext, context: BattleDecisionContext): Map<String, Any?> {
        ...
        val root = linkedMapOf<String, Any?>(
            ...
            "memory" to linkedMapOf(
                ...
                "switchPressure" to context.memory.switchPressure,
        ...
        linkedMapOf(
            ...
            "switchTargetPublicFacts" to PublicSwitchTypeFactsCalculator.forCandidate(candidate, context)?.let { facts ->
                linkedMapOf(
                    ...
                    // What the incoming Pokemon actually gets hit with, worst first. The type-chart
                    // entries above are a statement about typing; these are the same Showdown base
                    // projection the move candidates carry, from moves the battle has revealed.
                    // Without them a switch into a resisted type that still takes seventy percent
                    // read identically to one that takes twenty - and this brain is told, correctly,
                    // never to invent the number itself.
                    "incomingRevealedMoveThreats" to PublicSwitchIncomingThreatCalculator
                        .forCandidate(candidate, context)
                        .map { threat ->
                            linkedMapOf(
                                "attacker" to threat.attackerActiveSlot?.let { "opponent$it" },
                                "moveId" to threat.moveId,
                                "typeId" to threat.typeId,
                                "typeChartMultiplier" to threat.typeChartMultiplier,
                                "standardDamageFractionRange" to threat.damageFractionRange,
                                "standardKnockoutAssessment" to threat.knockoutAssessment?.name,
                            )
                        },
                )
            },
            ...
            "stallingProtection" to candidate.takeIf {
                LocalStallingProtectionRules.isStallingProtection(it)
            }?.let {
                val consecutiveSuccessfulUses = LocalStallingProtectionRules.consecutiveSuccessfulUses(
                    context.state,
                    BattleSide.ALLY,
                    candidate.actorSlot,
                )
                linkedMapOf(
                    "consecutiveSuccessfulUses" to consecutiveSuccessfulUses,
                    "nextSuccessProbability" to LocalStallingProtectionRules.nextSuccessProbability(
                        consecutiveSuccessfulUses,
                    ),
                )
            },
