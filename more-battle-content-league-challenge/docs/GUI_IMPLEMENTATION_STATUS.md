...
두 스파이크는 같은 고정 폭 행동 버튼, 같은 글자 크기와 유사한 직사각형 표면을 사용했다. 따라서 owo가 커스텀 표면을 허용한다는 점만 확인했다. 이후 독립 `cobblemon-ui-kit` 모듈에서 내용 폭·채움·고정 폭, 의미 variant, 세 크기, 아이콘 슬롯, 보조 문구, 여섯 상태와 불변 테마 스냅샷을 구현했다. 목록 surface·스크롤·진행 바는 Component Gallery에 들어갔지만 탭·콤보박스·알림·외부 cosmetic 테마 검증은 아직 구현하지 않았다.
...
- 프레임 시간과 입력 지연
- 탭·콤보박스·알림 등 남은 공용 위젯 계열, 외부 cosmetic 테마와 전체 폴백
...
4. 두 번째 League 화면에서 반복된 요구를 확인한 뒤 탭·카드·목록·알림 등 필요한 위젯만 공용 계약으로 승격한다.
