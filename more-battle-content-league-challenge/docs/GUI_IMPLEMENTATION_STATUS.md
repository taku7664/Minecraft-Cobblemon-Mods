...
1. 리소스팩 사용자 정의 관장 스킨과 `default`/`slim` 모델을 같은 슬롯에서 검증한다.
2. 물리 마우스 입력과 화면 읽기 프로그램 음성 출력을 수동 확인한다.
3. 프레임 시간과 입력 지연을 같은 조건에서 측정한다.
4. 근거를 모아 백엔드와 첫 시각 방향을 결정한다.
5. owo 채택 시 구현과 내레이션 전달 어댑터를 MBC 비공개 렌더러로 이동하고, 기각 시 owo 스파이크를 제거한다.
6. 미선택 구현과 개발용 직접 의존성을 제거한 뒤 체육관 상세 화면으로 확장한다.
