...
League Challenge는 독립된 Cobblemon UI 툴킷의 첫 실제 소비자다. 타입 있는 위젯, 레이아웃, 입력·포커스, 테마 계약과 Minecraft 렌더러는 공용 클라이언트 소스 모듈이 소유한다(MUST). League Challenge는 리그 상태 모델, 리그 화면 조립, 전용 컴포넌트, 테마와 자산만 소유한다(MUST).

League가 owo UI, `GuiGraphics`나 렌더 백엔드 내부 레이아웃 클래스를 직접 참조하면 안 된다(MUST NOT). 실제 백엔드가 바뀌어도 League의 상태·행동 계약은 유지돼야 한다(MUST). 화면 구조, 행동 ID, 확인 단계와 포커스 순서는 컴파일된 코드가 소유하며 리소스팩에서 읽지 않는다(MUST).
...
타이틀 화면 개발 fixture는 렌더 후보 비교 자료일 뿐이다. 다음 스파이크는 Component Gallery와 실제 월드에서 터미널 우클릭·서버 패킷으로 여는 `리그 홈`이다(MUST). 이 화면에서 위젯 체계와 시각 방향을 승인한 뒤 `리그 홈 → 체육관 상세 → 파티 확인 → 결과` 네 화면 수직 단면으로 확장한다(MUST). 웹 시안 승인을 선행 조건으로 두지 않는다(MUST NOT).
...
각 상태는 색뿐 아니라 아이콘, 형태와 번역 문자열로도 구분한다(MUST). 두 개 이상의 실제 화면에서 반복되지 않은 요구를 공용 UI 툴킷 API로 승격하지 않는다(SHOULD NOT).

공용 원시 위젯은 최소한 내용 폭·채움·고정 폭, 의미 variant, 크기, 아이콘, 제목과 보조 문구를 구분해야 한다(MUST). 모든 행동을 같은 넓이, 같은 글자 크기와 같은 표면으로 표현하면 안 된다(MUST NOT). 목록, 콤보박스와 드래그앤드롭은 실제 League 또는 Battle UI 사용처가 생길 때 승격한다(SHOULD).
...
첫 테마는 개별 화면에 색을 직접 박지 않고, 검증된 불변 테마 스냅샷에서 최소 다음 cosmetic 토큰을 사용한다(MUST).
...
- 텍스처: 프레임, 카드, 버튼의 normal/hover/focus/pressed/disabled/selected 상태, 아이콘과 초상화 슬롯 ID
...
첫 렌더 비교는 타이틀 화면 fixture에서 `320×240`, `426×240`, `640×360`으로 수행할 수 있다(MAY). 제품 승인은 `client.level`과 `client.player`가 있는 실제 월드에서 같은 크기·언어·상태를 다시 검증해야 한다(MUST). 웹 도구는 Minecraft 컴포넌트 표면이 안정된 뒤 반복 속도 이득이 확인될 때만 만든다(MAY). 만들 경우 같은 fixture와 내보낸 컴포넌트·상태·행동 manifest를 사용하지만 CSS 배치와 Minecraft 픽셀 배치의 동등성을 약속하지 않는다(MUST NOT).
...
| LGUI-2 | 코드 드로잉과 owo 후보의 타이틀 화면 개발 fixture | 세 폭·한영·fixture 3D·주입 입력·클리핑 비교 자료 확보 |
| LGUI-3 | 공용 Component Gallery와 실제 월드 League 홈 | 터미널 우클릭·서버 패킷·물리 입력·접근성·성능 검토 통과 |
...
`badges_3` fixture의 코드 드로잉과 owo 후보를 Minecraft 클라이언트의 타이틀 파노라마 위에서 `320×240`, `427×240`, `640×360`으로 비교했다. 두 후보는 같은 정보 구조와 반응형 배치를 유지했고, owo도 기본 위젯 외형 대신 League 전용 surface와 renderer를 적용할 수 있었다. 영어·한국어 화면과 주입된 키보드 포커스·행동·내레이션 항목·닫기 경로를 각각 확인했다. 이어 `long_disabled` fixture를 `427×240` 한영으로 비교해 긴 관장명·보상명, 포커스 가능한 잠긴 행동과 fixture 3D 모델의 scissor 경계까지 확인했다. 캡처와 차이는 [UI_BACKEND_SPIKE_RESULT.md](UI_BACKEND_SPIKE_RESULT.md)에 기록했다.

이는 실제 월드나 LGUI-3 증거가 아니다. 실제 리소스팩 관장 스킨과 `default`/`slim` 모델, 터미널·패킷 진입, 물리 마우스·키보드·음성 출력과 성능 증거가 남아 있다. 두 후보 모두 고정 폭·동일 글자 크기·동일한 행동 표면을 반복했으므로 위젯 프레임워크의 품질도 증명하지 않았다. owo 기본 어댑터는 내부 위젯 내레이션을 바닐라 화면으로 전달하지 않았으므로, 채택 시 툴킷 비공개 백엔드가 전달 어댑터를 소유해야 한다(MUST). 비교를 위한 League의 직접 owo 참조는 일회성 개발 예외이며 제품 단계 전에 툴킷 비공개 렌더러로 이동하거나 제거해야 한다(MUST).

## 8. 시각·기능 검증

- 세 논리 화면 크기의 동일 상태를 타이틀 fixture와 실제 월드에서 각각 나란히 비교하고 증거 등급을 구분한다.
...
- 빌드, 웹 스냅샷, fixture manifest와 타이틀 화면 캡처는 실제 월드의 터미널·패킷·물리 입력·성능 증거로 보고하지 않는다.

## 9. 구현 전에 남은 선택

1. Component Gallery와 실제 월드 화면으로 채택할 렌더링 백엔드
...
7. 공용 툴킷을 별도 필수 JAR, Jar-in-Jar 또는 빌드 시 공유 중 어떤 방식으로 배포할지

1번과 2번은 Component Gallery와 실제 월드 League 홈을 본 뒤 빡대리님이 결정한다. 구현자는 임의 팔레트, 제3자 자산이나 웹 시안을 확정본으로 넣으면 안 된다(MUST NOT).
