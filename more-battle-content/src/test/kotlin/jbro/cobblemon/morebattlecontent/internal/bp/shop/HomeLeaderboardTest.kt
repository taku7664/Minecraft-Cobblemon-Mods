...
class HomeLeaderboardTest {
    @Test
    fun `tower ranking orders best rank progress and wins with shared places`() {
        ...
        val delta = UUID(0, 4)
        val records = listOf(
            stats(gamma, category, rank = 9, progress = 4, wins = 90),
            stats(delta, category, rank = 10, progress = 1, wins = 30),
            stats(beta, category, rank = 10, progress = 2, wins = 40),
            stats(alpha, category, rank = 10, progress = 2, wins = 40),
        )
        ...
        assertEquals(listOf(10L, 10L, 10L, 9L), result.map(HomeLeaderboardEntry::highestRank))
    }
    ...
    fun `missing profile name falls back to short uuid`() {
        ...
        val result = HomeLeaderboard.project(listOf(stats(playerId, category, 1, 0, 1))) { null }
    ...
    private fun stats(
        ...
        totalWins = wins,
        progressMetrics = mapOf(BattleRecordMetrics.RANK_PROGRESS to progress),
        bestMetrics = mapOf(BattleRecordMetrics.HIGHEST_RANK to rank),
    )
