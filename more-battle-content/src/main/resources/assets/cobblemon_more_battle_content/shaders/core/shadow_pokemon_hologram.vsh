...
void main() {
    ...
    // The terrain hologram rebuilds a normal from screen derivatives because it only has a depth
    // buffer. A model already carries one, so the shading normal stays in model space: the key
    // light then sits still relative to the Pokemon instead of swinging as the camera orbits.
    modelNormal = normalize(Normal + vec3(0.000001));
