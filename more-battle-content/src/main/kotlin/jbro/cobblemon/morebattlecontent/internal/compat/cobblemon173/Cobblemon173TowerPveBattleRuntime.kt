...
internal class Cobblemon173TowerPveBattleRuntime(
    ...
    internal fun startManaged(prepared: Cobblemon173ManagedAiBattle): TowerBattleLaunchResult {
        ...
        if (!jbro.cobblemon.morebattlecontent.api.access.BattleContentAccess.allow(
                player, prepared.contentId, jbro.cobblemon.morebattlecontent.api.access.ContentAccessAction.START,
            )) return TowerBattleLaunchResult.Unavailable
        ...
        try {
            ...
            MoreBattleContent.LOGGER.error("{} lifecycle registration failed for {}", prepared.diagnosticsLabel, player.uuid, failure)
            return TowerBattleLaunchResult.Unavailable
        } catch (failure: LinkageError) {
            MoreBattleContent.LOGGER.error("{} lifecycle registration failed for {}", prepared.diagnosticsLabel, player.uuid, failure)
            return TowerBattleLaunchResult.Unavailable
